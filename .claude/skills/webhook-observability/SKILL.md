...
```typescript
// ❌ WRONG - Raw logger loses context
...
// ✅ CORRECT - Webhook logger with full context
...
Every handler MUST follow the `start()` → `success()`/`failure()` lifecycle:
...
```typescript
// ❌ WRONG - Missing lifecycle methods
const log = createWebhookLogger({ /* ... */ });
// No log.start()!
...
log.start();
// ... long operation ...
log.success({ result });  // Automatically logs: "⏱️ [SLOW] ... completed in 6234ms"
```
...
|--------|---------|--------|
| `🪝` | Handler start | `log.start()` |
| `🔍` | Debug/trace info | `log.debug()` |
| `✅` | Success | `log.success()` or `log.info()` |
| `⚠️` | Warning (non-fatal) | `log.warn()` |
| `❌` | Error/failure | `log.failure()` or `log.error()` |
| `⏱️` | Slow operation | Automatic in `log.success()` |
...
export async function handleEventName(
  ...
  try {
    ...
    try {
      ...
      log.warn({ err: sideEffectError }, "⚠️ Side effects failed (non-critical)");
    }
